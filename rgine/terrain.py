...
class _terrainR(_terrainBase):
	...
	def readTerrain(self, fname):
		...
		if raw2struct(ver, uint64).uint != raw2struct(_version_, uint64).uint:
			hFile.close()
			raise exception.GeneralException(getName(), getLine(), "Terrain File Version Mismatched.  ")
		self._data = []
		...
		for x in range(tx):
			...
			for y in range(ty):
				...
				if not d:
					self._data = []
					hFile.close()
					raise exception.GeneralException(getName(), getLine(), "Terrain File Size Mismatched.  ")
	...
	def readTextureFromFile(self, fname):
		...
		if raw2struct(ver, uint64).uint != raw2struct(_version_, uint64).uint:
			hFile.close()
			raise exception.GeneralException(getName(), getLine(), "Texture File Version Mismatched.  ")
		tx, ty = raw2struct(tx, uint64).uint, raw2struct(ty, uint64).uint
